...
{
    // --- ambient 环境光 ---
    vec3 f_ambient = u_Light.ambient * vec3(texture(u_material.texture_Ambient, m_TexCoord));

    // --- diffuse 漫反射 ---
    vec3 norm = normalize(m_Normal);
    vec3 lightDir = normalize(u_Light.lightPos - m_FragPos);
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 f_diffuse = u_Light.diffuse * diff * vec3(texture(u_material.texture_Ambient, m_TexCoord));
    ...
    if(diff > 0.0) // 只有当面朝向光源时才计算高光
    {
        ...
        f_specular = u_Light.specular * spec * vec3(texture(u_material.texture_Specular, m_TexCoord));
    }
