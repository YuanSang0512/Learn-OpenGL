...
namespace test
{
        ...
        ambientLight(0.4, 0.4, 0.4), diffuseLight(0.4, 0.4, 0.4), specularLight(0.5, 0.5, 0.5)
    {
        ...
        std::array<skyVertex, 24> vertices_skyBox;
        skyVertex* skyBoxBuffer = vertices_skyBox.data();
        skyBoxBuffer = basicModels.CreateSkyBoxVertexs(skyBoxBuffer, 2000.0f);
        std::vector<unsigned int> indices_skyBox(36);
        indices_skyBox = basicModels.CreateSkyBoxIndices(indices_skyBox);

        //生成立方体模型数据
        std::array<vertex, 24> vertices_object;
        vertex* ObjectBuffer = vertices_object.data();
        ObjectBuffer = basicModels.CreateCubeVertexs(ObjectBuffer, 300.0f);
		...
		m_SkyBoxVBO = std::make_unique<VertexBuffer>(vertices_skyBox.data(), 24 * sizeof(vertex), false);
		...
		m_ObjectShader = std::make_unique<Shader>("E:/VS project/openGL/openGL/res/shaders/Cube.shader");
